...
struct OperationalRegisters {
    ...
    crcr: Volatile<u64>,
    rsvdz: [u64; 2],
    dcbapp: Volatile<*const RawDeviceContextBaseAddressArray>,
    config: Volatile<u64>,
}
...
impl OperationalRegisters {
    ...
    fn page_size(&self) -> Result<usize> {
        ...
        if page_size_bits.count_ones() != 1 {
            return Err("PAGE_SIZE hasmultiple bits set");
        }
    ...
    fn set_dcbaa_ptr(
        ...
        self.dcbapp.write(dcbaa.inner_mut_ptr());
        Ok(())
    }
...
impl ScratchpadBuffers {
    fn alloc(
        ...
        info!("xhci: original nujm_scratchpad_bufs = {num_scratchpad_bufs}");
...
impl GenericTrbEntry {
    ...
    fn set_trb_type(&mut self, trb_type: TrbType) {
        self.controll.write_bits(10, 6, trb_type as u32).unwrap();
    }
    fn set_toggle_cycle(&mut self, value: bool) {
        self.controll.write_bits(1, 1, value.into()).unwrap()
    }
}
