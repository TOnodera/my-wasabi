...
fn efi_main(_image_handle: EfiHandle, efi_system_table: &EfiSystemTable) {
    ...
    fill_rect(&mut vram, 0x00ff00, 128, 128, 128, 128).expect("fill_rect failed.");
...
trait Bitmap {
    ...
    unsafe fn unchecked_pixel_mut(&mut self, x: i64, y: i64) -> *mut u32 {
        self.buf_mut()
    ...
    fn pixel_at_mut(&mut self, x: i64, y: i64) -> Option<&mut u32> {
        if self.is_in_x_range(x) && self.is_in_y_range(y) {
            unsafe { Some(&mut *self.unchecked_pixel_mut(x, y)) }
        } else {
            None
        }
    }
...
fn draw_font_fg<T: Bitmap>(buf: &mut T, x: i64, y: i64, color: u32, c: char) {
    if let Some(font) = look_up_font(c) {
...
fn look_up_font(c: char) -> Option<[[char; 8]; 16]> {
    let FONT_SOURCE: &str = include_str!("./font.txt");
