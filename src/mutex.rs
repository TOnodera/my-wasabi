...
pub struct Mutex<T> {
    ...
    createdj_at_line: u32,
}
impl<T: Sized> Debug for Mutex<T> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "Mutex @ {}:{}", self.created_file, self.createdj_at_line)
    }
}
impl<T: Sized> Mutex<T> {
    ...
    pub const fn new(data: T) -> Self {
        Self {
            ...
            createdj_at_line: Location::caller().line(),
        }
    }
    #[track_caller]
    ...
    pub fn lock(&self) -> MutexGuard<T> {
        ...
        panic!(
            "Failed tot lock Mutex at {}:{}, caller: {:?}, taker_line_num: {}",
            self.created_file,
            self.createdj_at_line,
            Location::caller(),
